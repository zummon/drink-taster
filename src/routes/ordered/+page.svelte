...
<div class="max-w-4xl mx-auto my-12 px-4">
	<div class="glass-card p-10">
		<h1 class="text-4xl font-black text-earth-brown mb-8 border-b border-soft-sage/20 pb-6 flex items-center gap-4">
			<span class="text-rustic-terracotta">🧺</span> Your Countryside Order
		</h1>

		{#if cart.items.length === 0}
			<div class="text-center py-20" in:fade>
				<p class="text-2xl font-bold text-earth-brown/40 mb-8">Your basket is empty...</p>
				<a href="/product" class="btn-primary inline-block">
					Browse Menu
				</a>
			</div>
		{:else}
			<div class="space-y-6 mb-12">
				{#each cart.items as item (item.orderId)}
					...
					>
						<img src={item.imgSrc} alt={item.imgAlt} class="w-24 h-24 object-cover rounded-xl shadow-sm" />
						
						<div class="flex-grow">
							<h3 class="text-xl font-bold text-earth-brown">{item.name}</h3>
							<p class="text-sm text-soft-sage font-black uppercase tracking-widest">{item.category}</p>
						</div>
						
						<div class="text-right">
							<p class="text-2xl font-black text-rustic-terracotta">฿{item.price}</p>
							<button 
			...
			<div class="bg-soft-sage/10 p-8 rounded-3xl border border-soft-sage/20">
				<div class="flex justify-between items-center mb-8">
					...
					<span class="text-4xl font-black text-rustic-terracotta">฿{cart.total}</span>
				</div>
				
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
					<button 
						onclick={() => { alert('Thank you for your order! It will be ready soon.'); cart.clear(); }}
